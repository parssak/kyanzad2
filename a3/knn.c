                   ...
                   double (*fptr)(Image *, Image *), int p_in, int p_out)
{
    ...
    int last = start_idx + N;
    // printf("child got: %d \n", N);

    for (int a = start_idx; a < last; a++)
    {
        Image img = testing->images[a];
        int res = knn_predict(training, &img, K, fptr);
        // int res = 0;
        if (res == testing->labels[a])
        {
            num_correct++;
        }
        // if (last == 50) {
...
double distance_cosine(Image *a, Image *b)
{
    ...
    //TODO
    double ab_sum = 0;
    double a_square_sum = 0;
    double b_square_sum = 0;

    for (int i = 0; i < NUM_PIXELS; i++)
    {
        ab_sum += a->data[i] * b->data[i];
        a_square_sum += a->data[i] * a->data[i];
        b_square_sum += b->data[i] * b->data[i];
    }
    
    return 2 * acos(ab_sum / (sqrt(a_square_sum) * sqrt(b_square_sum))) / M_PI;
}
