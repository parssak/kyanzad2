...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    int numRead = read(client_fd, &buffer, BUF_SIZE);

    
    buffer[numRead > 0 && buffer[numRead - 1] == '\0' ? numRead - 1 : numRead] = '\0';

    usernames[user_index].sock_fd = client_fd;
    usernames[user_index].username = buffer;
    return client_fd;
}
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    char buffer[BUF_SIZE + 1];
    char result[2 * BUF_SIZE + 3];
    char *username = usernames[client_index].username;

    int num_read = read(fd, &buffer, BUF_SIZE);
    buffer[num_read] = '\0';

    strncpy(result, username, strlen(username) + 1);
    strncat(result, " :", BUF_SIZE - strlen(result));

    result[strlen(username) + 2 > BUF_SIZE ? BUF_SIZE : strlen(usernames[client_index].username) + 2] = '\0';
    strncat(result, buffer, 2 * BUF_SIZE + 3 - strlen(result));

    result[2 * BUF_SIZE + 2] = '\0';
    ...
    for (int i = 0; i < MAX_CONNECTIONS; i++)
        if (usernames[i].sock_fd != -1)
            if (write(usernames[i].sock_fd, result, strlen(result)) != strlen(result))
